...
export class Connection {
  ...
  private makeUri(host: string, isRelay = false, relayOffset = 0): string {
    // RustDesk servers use raw WebSocket on port offsets from the base port:
    //   rendezvous: ws://host:(base+2)
    //   relay:      ws://host:(base+3) or ws://host:(base+relayOffset)
    let hostname: string;
    let port: number;
    ...
    if (host.includes(':')) {
      ...
      hostname = h;
      port = parseInt(p) + (isRelay ? (relayOffset || 3) : 2);
    } else {
      hostname = host;
